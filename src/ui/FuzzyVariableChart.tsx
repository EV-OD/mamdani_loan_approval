...
export const FuzzyVariableChart: React.FC<Props> = ({ 
  ...
  return (
    <div className="bg-white rounded-xl border border-gray-100 p-4 shadow-sm">
      ...
      {/* Membership Degrees Display */}
      <div className="mt-4 pt-4 border-t border-gray-50 flex flex-wrap gap-2">
        {variable.memberships.map((mf, i) => {
          const degree = currentValue !== undefined ? evaluateMembership(currentValue, mf) : 0;
          const color = colors[i % colors.length];
          return (
            <div key={mf.name} className="flex flex-col items-center flex-1 min-w-[70px]">
              <span className="text-[9px] font-black uppercase tracking-tighter text-gray-400 mb-1 truncate w-full text-center">
                {mf.name}
              </span>
              <div 
                className="text-[11px] font-mono font-bold px-2 py-1 rounded-lg border w-full text-center transition-all duration-500"
                style={{ 
                  borderColor: degree > 0 ? color : '#F3F4F6',
                  backgroundColor: degree > 0 ? `${color}10` : '#F9FAFB',
                  color: degree > 0 ? color : '#9CA3AF',
                  boxShadow: degree > 0 ? `0 0 10px ${color}20` : 'none'
                }}
              >
                μ={degree.toFixed(2)}
              </div>
